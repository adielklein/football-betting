...
// מעבר בין בהיר, כהה, ולפי המערכת.
//
// שלושה מצבים ולא מתג דו-מצבי: ברירת המחדל היא לכבד את הטלפון, ומי שרוצה
// לכפות משהו אחר יכול. לחיצה מקדמת למצב הבא, כדי לא לגזול מקום בכותרת.
//
// הכפתור יושב על רקע הכותרת הצבעונית ולכן צבעיו לבנים-שקופים ולא מהטוקנים.

const NEXT = { system: 'light', light: 'dark', dark: 'system' };
const ICON = { system: '🌗', light: '☀️', dark: '🌙' };
const LABEL = { system: 'לפי המכשיר', light: 'מצב בהיר', dark: 'מצב כהה' };

function ColorSchemeToggle({ compact = false }) {
