...
function NotificationSettings({ user }) {
  ...
  // 🔧 FIX: עדכון unsubscribe לתמוך במספר מכשירים
  const unsubscribeFromPush = async () => {
    ...
    try {
      ...
      // 🔧 FIX: שמור את ה-endpoint לפני שמוחקים את ה-subscription
      let endpoint = null;
      if (subscription) {
        endpoint = subscription.endpoint;
        console.log('📕 [NS] Endpoint to remove:', endpoint.substring(0, 50) + '...');
        await subscription.unsubscribe();
        console.log('✅ [NS] Browser subscription removed');
      }
      
      // 🔧 FIX: שלח את ה-endpoint לbackend כדי להסיר רק את המכשיר הזה
      ...
      console.log('📕 [NS] Unsubscribe response status:', response.status);
      
      if (response.ok) {
        const result = await response.json();
        console.log('✅ [NS] Server unsubscribe successful:', result);
        setIsSubscribed(false);
        
        // 🔧 FIX: הצג הודעה מותאמת לפי מספר המכשירים שנותרו
        if (result.devicesRemaining > 0) {
          alert(`✅ המכשיר הזה הוסר בהצלחה!\nעדיין יש לך ${result.devicesRemaining} מכשירים רשומים.`);
        } else {
          alert('✅ התראות בוטלו בהצלחה!');
        }
      } else {
        const errorText = await response.text();
        console.error('❌ [NS] Unsubscribe failed:', errorText);
        throw new Error('Failed to unsubscribe: ' + errorText);
      }
      
      console.log('📕 [NS] Unsubscribe process completed');
      
    } catch (error) {
      console.error('❌ [NS] Error in unsubscribe process:', error);
  ...
  const updateSettings = async () => {
    try {
      ...
      console.log('⚙️ [NS] New hoursBeforeLock:', hoursBeforeLock);
      ...
      if (response.ok) {
        const result = await response.json();
        console.log('✅ [NS] Settings updated:', result);
  ...
  const sendTestNotification = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // 🔧 FIX: הצג מספר מכשירים שקיבלו
        if (result.sent > 1) {
          alert(`✅ התראת בדיקה נשלחה ל-${result.sent} מכשירים!\nבדוק את כל המכשירים שלך.`);
        } else {
        ...
        <div style={{ 
          ...
          <div style={{ 
            ...
            <p style={{ marginTop: '0.5rem', fontSize: '11px', color: '#666' }}>
              ...
              📱 תמיכה במספר מכשירים: אפשר להפעיל על המחשב והפלאפון - כולם יקבלו!
            </p>
          </div>
        </div>
      )}
